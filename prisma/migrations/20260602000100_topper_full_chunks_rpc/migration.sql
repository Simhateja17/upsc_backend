...
AS $$
  ...
  WHERE
    ...
    AND e.chunk_type IN ('full', 'question', 'answer')
    AND a.usable_for_rag = TRUE
    AND a.quality_status IN ('gold', 'silver')
    AND NULLIF(BTRIM(a.question_text), '') IS NOT NULL
    AND BTRIM(a.student_answer_text) <> ''
  ...
  ORDER BY
    CASE e.chunk_type WHEN 'full' THEN 0 WHEN 'answer' THEN 1 ELSE 2 END,
